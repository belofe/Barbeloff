...
class Intermediate(commands.Cog):
    ...
    async def intermediate(self, ctx):
        ...
        class WorkoutView(discord.ui.View):
            def __init__(self, intermediate_cog):
                super().__init__(timeout=60.0)
                self.intermediate_cog = intermediate_cog
            ...
            async def handle_selection(self, interaction, workout_type):
                ...
                if workout_type == "strength":
                    ...
                    await self.intermediate_cog.intermediate_strength(ctx)
                elif workout_type == "lean":
                    await interaction.followup.send("**🏃 Lean Program Selected**")
                    await self.intermediate_cog.intermediate_lean(ctx)
                else:
                    await interaction.followup.send("**⚖️ Hybrid Program Selected**")
                    await self.intermediate_cog.intermediate_hybrid(ctx)
                self.stop()
    ...
    @commands.command()
    async def intermediate_strength(self, ctx):
...
"""
    ...
    @commands.command()
    async def intermediate_lean(self, ctx):
        """Get a beginner workout routine"""
        workout = """
**For each exercise, do at least 1 warm up set for 8 - 12 reps using approximately 50% of your working set weight** 
...
"""
    ...
    @commands.command()
    async def intermediate_hybrid(self, ctx):
        """Get a beginner workout routine"""
        workout = """
